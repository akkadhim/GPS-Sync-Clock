...
void main(){
  ...
  while(1) {
    OERR1_bit = 0;                 // Set OERR to 0  overrun error bit in uart
    FERR1_bit = 0;                 // Set FERR to 0  framming error bit in uart

    if(ready == 1) {               // If the data in txt array is ready do:
      ready = 0;
      string = strstr(txt,"$GPGLL");   //locates the first position of $GPGLL in txt
